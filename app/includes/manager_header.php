<?php
/**
 * Manager Header Component — reads real notifications from furn_notifications
 */
if (!isset($_SESSION['user_id']) || $_SESSION['user_role'] !== 'manager') { return; }

$managerId   = $_SESSION['user_id'];
$managerName = $_SESSION['first_name'] ?? $_SESSION['user_name'] ?? 'Manager';
$pageTitle   = $pageTitle ?? 'Manager Portal';
...
try {
    ...
    $s->execute([$managerId]);
    $dbNotifications = $s->fetchAll(PDO::FETCH_ASSOC);
} catch (PDOException $e) {}

// Quick Alerts (KPI counts)
$kpi = ['pending_orders' => 0, 'pending_tasks' => 0, 'unread_messages' => 0, 'pending_payments' => 0, 'open_complaints' => 0];
try {
    $kpi['pending_orders']   = (int)$pdo->query("SELECT COUNT(*) FROM furn_orders WHERE status IN ('pending_cost_approval','pending_review')")->fetchColumn();
    $kpi['pending_tasks']    = (int)$pdo->query("SELECT COUNT(*) FROM furn_production_tasks WHERE status='pending'")->fetchColumn();
    try { $s=$pdo->prepare("SELECT COUNT(*) FROM furn_messages WHERE receiver_id=? AND is_read=0"); $s->execute([$managerId]); $kpi['unread_messages']=(int)$s->fetchColumn(); } catch(PDOException $e2){}
    try { $kpi['pending_payments']=(int)$pdo->query("SELECT COUNT(*) FROM furn_payments WHERE status='pending'")->fetchColumn(); } catch(PDOException $e2){}
    try { $kpi['open_complaints']=(int)$pdo->query("SELECT COUNT(*) FROM furn_complaints WHERE status='open'")->fetchColumn(); } catch(PDOException $e2){}
} catch (PDOException $e) {}

$csrf_token = $_SESSION[CSRF_TOKEN_NAME] ?? $_SESSION['csrf_token'] ?? '';
$badgeCount = $unreadCount;
$initials   = strtoupper(substr($managerName, 0, 1));
        ...
        <div style="position:relative;cursor:pointer;" onclick="toggleNotificationDropdown()" title="Notifications">
            ...
            <div id="notifDropdown" style="display:none;position:absolute;top:35px;right:0;background:white;border-radius:10px;box-shadow:0 4px 24px rgba(0,0,0,0.18);min-width:340px;max-width:380px;z-index:9999;">
                ...
                <div style="padding:14px 16px;border-bottom:1px solid #e9ecef;display:flex;justify-content:space-between;align-items:center;">
                    ...
                    <a href="<?php echo BASE_URL; ?>/public/manager/notifications" style="font-size:12px;color:#3498db;text-decoration:none;">View All</a>
                </div>
                ...
                <div style="padding:8px 16px;background:#fff8f0;border-bottom:1px solid #f0e0d0;">
                    ...
                    <div style="display:flex;flex-wrap:wrap;gap:6px;">
                        <?php if($kpi['pending_orders']>0): ?><a href="<?php echo BASE_URL; ?>/public/manager/orders" style="background:#fdecea;color:#e74c3c;padding:3px 9px;border-radius:10px;font-size:11px;font-weight:600;text-decoration:none;"><i class="fas fa-shopping-cart me-1"></i><?php echo $kpi['pending_orders']; ?> Orders</a><?php endif; ?>
                        <?php if($kpi['pending_tasks']>0): ?><a href="<?php echo BASE_URL; ?>/public/manager/production" style="background:#fff3cd;color:#856404;padding:3px 9px;border-radius:10px;font-size:11px;font-weight:600;text-decoration:none;"><i class="fas fa-tasks me-1"></i><?php echo $kpi['pending_tasks']; ?> Tasks</a><?php endif; ?>
                        <?php if($kpi['pending_payments']>0): ?><a href="<?php echo BASE_URL; ?>/public/manager/payments" style="background:#eafaf1;color:#27ae60;padding:3px 9px;border-radius:10px;font-size:11px;font-weight:600;text-decoration:none;"><i class="fas fa-credit-card me-1"></i><?php echo $kpi['pending_payments']; ?> Payments</a><?php endif; ?>
                        <?php if($kpi['open_complaints']>0): ?><a href="<?php echo BASE_URL; ?>/public/manager/orders" style="background:#fdecea;color:#e74c3c;padding:3px 9px;border-radius:10px;font-size:11px;font-weight:600;text-decoration:none;"><i class="fas fa-exclamation-circle me-1"></i><?php echo $kpi['open_complaints']; ?> Complaints</a><?php endif; ?>
                        <?php if($kpi['unread_messages']>0): ?><a href="<?php echo BASE_URL; ?>/public/manager/messages" style="background:#e8f4fd;color:#2980b9;padding:3px 9px;border-radius:10px;font-size:11px;font-weight:600;text-decoration:none;"><i class="fas fa-envelope me-1"></i><?php echo $kpi['unread_messages']; ?> Messages</a><?php endif; ?>
                    </div>
                </div>
                <?php endif; ?>
                ...
                <div style="padding:10px 14px;border-top:1px solid #e9ecef;display:flex;justify-content:space-between;align-items:center;">
                    ...
                    <a href="<?php echo BASE_URL; ?>/public/manager/notifications" style="color:#3498db;text-decoration:none;font-size:12px;font-weight:600;">View All →</a>
                </div>
            </div>
        </div>

        <!-- Profile -->
        <div style="display:flex;align-items:center;gap:10px;cursor:pointer;" onclick="window.location.href='<?php echo BASE_URL; ?>/public/manager/profile';">
            <div style="background:#9B59B6;width:35px;height:35px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-weight:700;color:white;font-size:16px;overflow:hidden;">
            ...
            <div>
                ...
                <div style="background:#9B59B6;padding:2px 8px;border-radius:10px;font-size:10px;font-weight:700;color:white;">MANAGER</div>
            </div>
        </div>
    </div>
</div>
