...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action']??'') === 'mark_all') {
    ...
    header('Location: '.BASE_URL.'/public/manager/notifications'); exit();
}
...
.notif-item:hover{background:#f8f9fa;}.notif-item.unread{background:#f0f8ff;border-left:3px solid #3498db;}.notif-item.unread .notif-title{font-weight:700;}
.notif-icon{width:40px;height:40px;border-radius:50%;display:flex;align-items:center;justify-content:center;flex-shrink:0;}
.filter-btn{padding:7px 16px;border-radius:20px;border:1.5px solid #ddd;background:white;cursor:pointer;font-size:13px;font-weight:600;color:#555;text-decoration:none;transition:all .2s;}
...
<div class="main-content">
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:20px;flex-wrap:wrap;gap:12px;">
        <div><h2 style="margin:0;color:#2c3e50;"><i class="fas fa-bell me-2"></i>Notifications</h2>
    ...
    <div style="display:flex;gap:8px;flex-wrap:wrap;margin-bottom:16px;align-items:center;">
        ...
        <form method="GET" style="display:flex;gap:8px;margin-left:auto;"><input type="hidden" name="filter" value="<?php echo htmlspecialchars($filter);?>"><input type="text" name="search" value="<?php echo htmlspecialchars($search);?>" placeholder="Search..." style="padding:7px 12px;border:1.5px solid #ddd;border-radius:8px;font-size:13px;width:200px;"><button type="submit" style="padding:7px 14px;background:#2c3e50;color:white;border:none;border-radius:8px;cursor:pointer;"><i class="fas fa-search"></i></button><?php if($search):?><a href="?filter=<?php echo $filter;?>" style="padding:7px 12px;background:#f0f0f0;border-radius:8px;font-size:13px;text-decoration:none;color:#555;display:flex;align-items:center;"><i class="fas fa-times"></i></a><?php endif;?></form>
    </div>
    <div class="section-card" style="padding:0;overflow:hidden;">
        <?php if(empty($notifications)):?><div style="text-align:center;padding:60px 20px;color:#7f8c8d;"><i class="fas fa-bell-slash" style="font-size:48px;opacity:.3;display:block;margin-bottom:16px;"></i><div style="font-size:16px;font-weight:600;">No notifications found</div></div>
        ...
        <a href="<?php echo htmlspecialchars($link);?>" onclick="markRead(<?php echo (int)$n['id'];?>,this)" class="notif-item <?php echo $n['is_read']?'':'unread';?>">
            ...
            <div style="flex:1;">
                ...
                <div style="font-size:11px;color:#aaa;"><i class="fas fa-clock me-1"></i><?php echo timeAgo($n['created_at']);?> &nbsp;·&nbsp; <span style="background:<?php echo $color;?>22;color:<?php echo $color;?>;padding:1px 7px;border-radius:8px;font-size:10px;font-weight:700;"><?php echo strtoupper($n['type']);?></span></div>
            </div>
    ...
    <?php if($pages>1):?><div style="display:flex;justify-content:center;gap:6px;margin-top:20px;flex-wrap:wrap;"><?php for($i=1;$i<=$pages;$i++):?><a href="?filter=<?php echo $filter;?>&page=<?php echo $i;?><?php echo $search?'&search='.urlencode($search):'';?>" style="padding:7px 13px;border-radius:7px;text-decoration:none;font-size:13px;font-weight:600;<?php echo $i===$page?'background:#3498db;color:white;':'background:white;color:#555;border:1.5px solid #ddd;';?>"><?php echo $i;?></a><?php endfor;?></div><?php endif;?>
</div>
<script>
