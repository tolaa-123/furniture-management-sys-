...
$adminName = $_SESSION['first_name'] ?? $_SESSION['user_name'] ?? 'Admin User';
...
// Fetch admin data with profile_image
...
<body>
    ...
    <?php include __DIR__ . '/../../../app/includes/admin_header.php'; ?>
    
    <div class="main-content">
        <div class="page-header">
            <h1 class="page-title"><i class="fas fa-user-circle"></i> My Profile</h1>
            <p style="color: #7f8c8d;">View and manage your administrator profile</p>
        </div>
            ...
            <div style="display: flex; align-items: center; gap: 30px; margin-bottom: 20px;">
                <div class="profile-avatar" id="avatarContainer">
                    <?php if (!empty($adminData['profile_image'])): ?>
                        <img src="<?php echo BASE_URL; ?>/public/uploads/profile_images/<?php echo htmlspecialchars($adminData['profile_image']); ?>" 
                             alt="Profile Picture" 
                             class="profile-avatar-img" 
                             id="avatarPreview">
                    <?php else: ?>
                        <span id="avatarInitials"><?php echo strtoupper(substr($adminData['first_name'] ?? 'A', 0, 1) . substr($adminData['last_name'] ?? 'D', 0, 1)); ?></span>
                    <?php endif; ?>
                ...
                <div>
                    <h2 style="margin-bottom: 5px;"><?php echo htmlspecialchars($adminData['first_name'] . ' ' . $adminData['last_name']); ?></h2>
                    <p style="color: #7f8c8d; margin: 0;"><i class="fas fa-envelope"></i> <?php echo htmlspecialchars($adminData['email']); ?></p>
                    <p style="color: #7f8c8d; margin: 0;"><i class="fas fa-shield-alt"></i> ADMINISTRATOR</p>
            ...
            <div id="viewMode">
                <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(250px, 1fr)); gap: 20px;">
                    <div>
                        ...
                        <div style="padding: 10px; background: #f8f9fa; border-radius: 8px; font-weight: 500;"><?php echo htmlspecialchars($adminData['username']); ?></div>
                    </div>
                    <div>
                        <label style="font-weight: 600; color: #7f8c8d; font-size: 13px; margin-bottom: 5px; display: block;">First Name</label>
                        <div style="padding: 10px; background: #f8f9fa; border-radius: 8px; font-weight: 500;"><?php echo htmlspecialchars($adminData['first_name']); ?></div>
                    </div>
                    <div>
                        <label style="font-weight: 600; color: #7f8c8d; font-size: 13px; margin-bottom: 5px; display: block;">Last Name</label>
                        <div style="padding: 10px; background: #f8f9fa; border-radius: 8px; font-weight: 500;"><?php echo htmlspecialchars($adminData['last_name']); ?></div>
                    </div>
                    <div>
                        <label style="font-weight: 600; color: #7f8c8d; font-size: 13px; margin-bottom: 5px; display: block;">Email Address</label>
                        <div style="padding: 10px; background: #f8f9fa; border-radius: 8px; font-weight: 500;"><?php echo htmlspecialchars($adminData['email']); ?></div>
                    </div>
                    <div>
                        <label style="font-weight: 600; color: #7f8c8d; font-size: 13px; margin-bottom: 5px; display: block;">Phone Number</label>
                        <div style="padding: 10px; background: #f8f9fa; border-radius: 8px; font-weight: 500;"><?php echo htmlspecialchars($adminData['phone'] ?? 'Not provided'); ?></div>
                    </div>
                    <div>
                        <label style="font-weight: 600; color: #7f8c8d; font-size: 13px; margin-bottom: 5px; display: block;">Member Since</label>
                        <div style="padding: 10px; background: #f8f9fa; border-radius: 8px; font-weight: 500;"><?php echo date('F d, Y', strtotime($adminData['created_at'])); ?></div>
                    </div>
                </div>
                ...
                <form id="profileForm">
                    <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(250px, 1fr)); gap: 20px;">
                        ...
                        <div>
                            ...
                            <input type="text" name="first_name" value="<?php echo htmlspecialchars($adminData['first_name']); ?>" style="width: 100%; padding: 10px; border: 2px solid #e9ecef; border-radius: 8px;" required>
                        </div>
                        <div>
                            <label style="font-weight: 600; color: #2c3e50; font-size: 13px; margin-bottom: 5px; display: block;">Last Name</label>
                            <input type="text" name="last_name" value="<?php echo htmlspecialchars($adminData['last_name']); ?>" style="width: 100%; padding: 10px; border: 2px solid #e9ecef; border-radius: 8px;" required>
                        </div>
                        <div>
                            <label style="font-weight: 600; color: #2c3e50; font-size: 13px; margin-bottom: 5px; display: block;">Email Address</label>
                            <input type="email" name="email" value="<?php echo htmlspecialchars($adminData['email']); ?>" style="width: 100%; padding: 10px; border: 2px solid #e9ecef; border-radius: 8px;" required>
                        </div>
                        <div>
                            <label style="font-weight: 600; color: #2c3e50; font-size: 13px; margin-bottom: 5px; display: block;">Phone Number</label>
                            <input type="tel" name="phone" value="<?php echo htmlspecialchars($adminData['phone'] ?? ''); ?>" style="width: 100%; padding: 10px; border: 2px solid #e9ecef; border-radius: 8px;" placeholder="+251-XXX-XXXXXX">
                        </div>
                        <div>
                            <label style="font-weight: 600; color: #7f8c8d; font-size: 13px; margin-bottom: 5px; display: block;">Member Since</label>
                            <div style="padding: 10px; background: #f8f9fa; border-radius: 8px; font-weight: 500;"><?php echo date('F d, Y', strtotime($adminData['created_at'])); ?></div>
        ...
        <div class="section-card">
            ...
            <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 15px;">
                <a href="<?php echo BASE_URL; ?>/public/admin/settings" style="padding: 15px; background: #f8f9fa; border-radius: 8px; text-decoration: none; color: #2c3e50; border-left: 4px solid #e67e22; transition: all 0.3s;" onmouseover="this.style.background='#e67e22'; this.style.color='white'" onmouseout="this.style.background='#f8f9fa'; this.style.color='#2c3e50'">
                    <i class="fas fa-cog"></i> System Settings
                </a>
                <a href="<?php echo BASE_URL; ?>/public/admin/users" style="padding: 15px; background: #f8f9fa; border-radius: 8px; text-decoration: none; color: #2c3e50; border-left: 4px solid #3498db; transition: all 0.3s;" onmouseover="this.style.background='#3498db'; this.style.color='white'" onmouseout="this.style.background='#f8f9fa'; this.style.color='#2c3e50'">
        ...
        .then(data => {
            if (data.success) {
                ...
                if (headerAvatar) {
                    ...
                    headerAvatar.alt = "<?php echo htmlspecialchars($adminData['first_name']); ?>";
                }
    ...
    document.getElementById('profileForm').addEventListener('submit', function(e) {
        ...
        fetch(BASE_URL + '/public/api/update_admin_profile.php', {
            method: 'POST',
            body: formData
        })
        .then(response => {
